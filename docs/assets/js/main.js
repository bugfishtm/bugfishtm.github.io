...
function initNav() {
  ...
  if (burger && menu) {
    ...
    // Following a link (e.g. an in-page #anchor on the same page)
    // must not leave the mobile menu hanging open over the content.
    menu.addEventListener("click", e => {
      if (e.target.closest("a[href]") && !e.target.closest(".has-dropdown > .nav-link")) setMenu(false);
    });
    // Tapping outside the open mobile menu closes it.
    document.addEventListener("click", e => {
      if (menu.classList.contains("open") && !e.target.closest("#nav-menu")) setMenu(false);
    });
    // Rotating a phone / widening the window past the breakpoint.
    window.matchMedia("(min-width: 901px)").addEventListener("change", ev => { if (ev.matches) setMenu(false); });
  }
...
/* ---- Deep links to a section --------------------------------
   The home sections (#news, #about, ...) are filled from JSON
...
/* ---- Legacy search links ------------------------------------
   ...
   dropped. Pages that carry their own search input (projects,
   tools) handle ?q= themselves. */
function forwardLegacySearch() {
  if (document.getElementById("project-search") || document.getElementById("tool-search")) return;
